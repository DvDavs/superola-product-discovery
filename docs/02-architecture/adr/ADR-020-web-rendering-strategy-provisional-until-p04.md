...
**P04 evidence:** of the **35 rendered V1 surfaces, 24 are `DOC` and 11 are `LOCAL`** — every one is document-and-navigation or carries only local interactive state that does not need to survive navigation. (`UX-35` is a notification body, not a rendered surface; `UX-37` and `UX-38` are `FUTURE` and deliberately unclassified.) The three islands named in the budget below span six of those surfaces; they are the only places where plain HTML is genuinely insufficient.
...
**The deciding factor is that the architecture already removed the reasons a rich client usually wins.** No realtime transport means no live thread. No availability model means no calendar widget. No rendered map (`ADR-019` Level 3, decided in `docs/04-ux/map-decision.md`) means no tile surface — and a map would have been the only plausible surface in the product requiring client state that survives navigation, so that decision and this one are linked and were taken on independent grounds.
