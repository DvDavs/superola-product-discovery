...
| **Operator queues** | `UX-26`–`UX-34` | **Operator queues are the one surface class allowed to be desktop-first, and P04 states that rather than assuming it.** They remain keyboard-operable and must not break at narrow width, but multi-column case tables, side-by-side case detail, and filter panels may be optimized for a wide viewport. | None material. The failure mode of the opposite choice is worse: phone-optimizing eight domain queues plus one cross-queue index spends the scarcest budget in the project on the smallest, most controllable, and most training-available audience. | LOW |
