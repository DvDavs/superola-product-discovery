...
| Q-026 | Must Superola be able to answer "who at Superola read this customer's request or conversation, and when" — and is that disclosed to users? | Read attribution is invasive to retrofit and P02 designs the audit surface. **David can decide this without owner input**, and it is low cost. | David + product. |
...
| Q-028 | May one person be both customer and provider under a single account, and does the answer change any disclosure, verification, or authorization rule? | A structural fork not previously in any register. A DJ hiring a photographer, or a venue owner planning a family event, is routine in this market. A hard account-type partition makes that case a migration. **P02 recommends the non-partitioned answer (`ADR-004`)**; David can ratify without owner input. | David + product; owner input on policy only. |
